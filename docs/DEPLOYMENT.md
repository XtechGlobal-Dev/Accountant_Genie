...
# Rotate AUTH_SECRET: change it on BOTH services in one sitting. It is read in one
...
**Rollback.** Render's rollback to a previous deploy reverts the app only; it does not revert a
migration. Write migrations so the previous app version still runs against the new schema — add
columns, do not rename them — or a rollback takes the app down.

---

## Appendix — environment variables

Four groups. **A** must be identical on both services. **B** is what the service serving HTTP
needs. **C** is worker-only. **D** applies to both. The web service takes A + B + D; the worker
takes A + C + D.

### A. Shared — identical on both services
...
AUTH_SECRET=<generate once, paste the same value into both services>
REDIS_URL=<wired by render.yaml from the keyvalue service; do not set by hand>
S3_BUCKET=
...
| `REDIS_URL` | Jobs run in-process, inside the request that started them — a deploy or a dropped connection kills an import mid-ledger-write |
