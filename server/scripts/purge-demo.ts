 ...
 * an asset, a loan and a subcontractor. Every one of those rows reaches the
 * firm through an `onDelete: Cascade` foreign key, so deleting the firm is the
 * whole job. Nothing else is touched:
 *
 *   - the system chart of accounts has no firm and stays
 *   - tax rule versions are global; a verifier from the demo firm is nulled
 *   - every other firm's rows are left exactly as they are, and the script
...
async function main() {
  ...
  try {
    ...
    // cascade that reached further than expected aborts the whole thing.
    const others = `select count(*)::int as n from "Client" where "firmId" <> $1`;
    const othersBefore = (await db.query(others, [DEMO_FIRM_ID])).rows[0].n;
    const before = (await db.query(COUNTS)).rows[0];

    await db.query("begin");
    try {
      const deleted = await db.query(`delete from "Firm" where id = $1 and name = $2`, [DEMO_FIRM_ID, DEMO_FIRM_NAME]);
      if (deleted.rowCount !== 1) throw new Error(`expected to delete one firm, deleted ${deleted.rowCount}`);

      const othersAfter = (await db.query(others, [DEMO_FIRM_ID])).rows[0].n;
      if (othersAfter !== othersBefore) throw new Error(`other firms' clients changed (${othersBefore} → ${othersAfter})`);
